...
export class Migration20260512131732 extends Migration {

  override async up(): Promise<void> {
    this.addSql(`create table if not exists "promotion_ext_config" ("id" text not null, "promotion_id" text not null, "auto_apply" boolean not null default false, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "promotion_ext_config_pkey" primary key ("id"));`);
    this.addSql(`CREATE INDEX IF NOT EXISTS "IDX_promotion_ext_config_deleted_at" ON "promotion_ext_config" ("deleted_at") WHERE deleted_at IS NULL;`);
    ...
    this.addSql(`alter table if exists "promotion_ext_rule_group" add constraint "promotion_ext_rule_group_promotion_config_id_foreign" foreign key ("promotion_config_id") references "promotion_ext_config" ("id") on update cascade;`);
  ...
  override async down(): Promise<void> {
    ...
    this.addSql(`drop table if exists "promotion_ext_config" cascade;`);
